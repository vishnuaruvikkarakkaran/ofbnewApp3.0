...
public class ExploreCoursesViewHolder extends BaseViewHolder<ExploreCourse, OnRecyclerViewClickListener<ExploreCourse>> {
    ...
    public ExploreCoursesViewHolder(@NonNull View itemView) {
        super(itemView);
        ...
        if (!TextUtils.isEmpty(item.getOldItemPrice()) && !TextUtils.isEmpty(item.getCourseDiscount())) {
            ...
            if (item.getItemIsFree().equals("1")) {
                ...
                if (oldPrice > 0 && discount > 0) {
                    coursePriceOld.setVisibility(View.VISIBLE);
            ...
            if (item.getCourseHasRating().equals("1")) {
                ...
                if (rating > 0)
                    ...
                    courseRatingBar.setRating(0);

            } else if (item.getCourseHasRating().equals("0")) {
                courseRatingBar.setVisibility(View.INVISIBLE);
            }

            int oldPrice1 = Integer.parseInt(item.getOldItemPrice());

            if (oldPrice1 > 0 && discount > 0)
                coursePriceOld.setText("₹ " + oldPrice1);
            else
                coursePriceOld.setText("");
